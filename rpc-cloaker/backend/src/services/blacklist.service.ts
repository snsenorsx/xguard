...
import { ThreatIntelligenceService, getThreatIntelligenceService } from './threatIntelligence.service';
...
export class BlacklistService {
  ...
  /**
   * Check if IP is blacklisted (with threat intelligence integration)
   */
  async isBlacklisted(ipAddress: string): Promise<boolean> {
    try {
      ...
      let isBlacklisted = localResult.rows.length > 0;

      // If not locally blacklisted, check threat intelligence
      if (!isBlacklisted) {
        try {
          const threatResult = await this.threatIntelligence.analyzeIP(ipAddress);
          
          if (threatResult.isMalicious && threatResult.confidence >= 75) {
            // Auto-blacklist high-confidence threats
            await this.addToBlacklist({
              ipAddress,
              reason: `Threat intelligence: ${threatResult.sources.map(s => s.name).join(', ')}`,
              detectionType: 'suspicious',
              confidenceScore: threatResult.confidence / 100,
          ...
          console.warn('Threat intelligence check failed:', threatError);
          // Continue without threat intel if service fails
        }
      }
  ...
  private getReputationDeduction(detectionType: string, confidenceScore: number): number {
    ...
    const baseDeduction = (baseDeductions as any)[detectionType] || 10;
