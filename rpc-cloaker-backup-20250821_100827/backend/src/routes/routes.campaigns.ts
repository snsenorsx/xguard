...
export default async function campaignsRoutes(server: FastifyInstance) {
  ...
  server.get('/', async (request, reply) => {
    ...
    const result = await db.query(
      `SELECT id, name, status, money_page_url AS "moneyPageUrl", safe_page_url AS "safePageUrl",
              redirect_type AS "redirectType", 
              money_page_redirect_type AS "moneyPageRedirectType",
              safe_page_redirect_type AS "safePageRedirectType",
  ...
  server.post('/', async (request, reply) => {
    ...
    const db = getDb();
    const result = await db.query(
      `INSERT INTO campaigns (user_id, name, money_page_url, safe_page_url, redirect_type, 
                              money_page_redirect_type, safe_page_redirect_type, notes)
      ...
      [
        ...
        body.moneyPageUrl,
        body.safePageUrl,
        body.redirectType || body.moneyPageRedirectType || '302',  // backward compatibility
        body.moneyPageRedirectType || '302',
        body.safePageRedirectType || 'direct',
        body.notes || null,
      ]
    );
  ...
  server.patch('/:id', async (request, reply) => {
    ...
    if (body.moneyPageRedirectType) { fields.push(`money_page_redirect_type = $${idx++}`); values.push(body.moneyPageRedirectType); }
    if (body.safePageRedirectType) { fields.push(`safe_page_redirect_type = $${idx++}`); values.push(body.safePageRedirectType); }
    ...
    values.push(id);
    values.push((request as any).user.id);
  ...
  // Streams - create
  server.post('/:id/streams', async (request, reply) => {
    const { id } = request.params as any;
    const body = request.body as any;
    const db = getDb();
    const res = await db.query(
  ...
  // Streams - update
  server.patch('/:id/streams/:streamId', async (request, reply) => {
  ...
  server.post('/:id/streams/:streamId/targeting', async (request, reply) => {
  ...
  server.patch('/:id/streams/:streamId/targeting/:ruleId', async (request, reply) => {
